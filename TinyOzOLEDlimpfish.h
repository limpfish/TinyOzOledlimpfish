...
public:
	...
	void printBiggerString(const char *String, byte X=255, byte Y=255, byte numChar=255);
	void sendBiggerCharData(char c, byte column);

	void printBiggerChar(char c, byte X=255, byte Y=255);
	...
	void setCursorXYPixel(byte Column, byte Row);
	...
	void scrollLeftPixel(byte start, byte end, byte speed);
